    ...
    <div className="ImagesHome">
      <div className="img-home">
        <div className="left-img">
          ...
          <GymButton data={data[0].msg} className="carousel-button1" style={style.carouselButton1} />
        </div>
        <div className="right-imgs">
          {data.slice(1).map((sub, index) => (
            <div key={index} className="images-div">
              <img src={sub.img} alt={sub.msg} className={`img${index + 2}`} />
              <GymButton data={sub.msg} className={`carousel-button${index + 2}`} style={style[`carouselButton${index + 2}`]} />
            </div>
          ))}
        </div>
      </div>
